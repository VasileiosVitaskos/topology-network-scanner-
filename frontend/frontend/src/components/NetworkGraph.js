...
export default function NetworkGraph({ scanResult, dataset }) {
  ...
  useEffect(() => {
    ...
    nodeGroup.append('circle')
      ...
      .attr('fill', d => {
        if (involved.has(d.id) || involved.has(d.ip)) return '#ef4444';
        return segColors[d.segment] || '#64748b';
      })
      .attr('stroke', d => {
        if (involved.has(d.id) || involved.has(d.ip)) return '#ef444480';
        return 'transparent';
      })
    ...
    nodeGroup.append('text')
      ...
      .attr('fill', d => (involved.has(d.id) || involved.has(d.ip)) ? '#ef4444' : '#475569')
      .attr('dx', 14)
      .attr('dy', 3);
    ...
    svg.append('g')
      ...
      .attr('font-size', 9)
      .attr('font-family', "'JetBrains Mono', monospace")
      .attr('fill', d => segColors[d] || '#475569')
      .attr('opacity', 0.5);
